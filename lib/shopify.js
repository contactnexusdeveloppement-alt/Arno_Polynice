 ...
 */
export async function getEthicsPage() {
  const query = `
    query getEthicsPage($type: String!) {
      metaobjects(type: $type, first: 1) {
        edges {
          node {
            fields { key value }
 ...
 */
export async function getEthicsValues() {
  const query = `
    query getEthicsValues($type: String!) {
...
export async function getPressPage() {
  const query = `
    query getPressPage($type: String!) {
      metaobjects(type: $type, first: 1) {
        edges {
          node {
            fields { key value }
 ...
 *   position (number), type (article|video), media_name, media_logo (file),
 *   title, excerpt, publish_date, url, thumbnail (file, optionnel)
 *
 * Les fichiers (logos, thumbnails) sont résolus en URLs via la query reference.
 * Retourne [] si aucune entrée (la page affiche alors un message d'invitation).
 */
export async function getPressItems() {
  const query = `
    query getPressItems($type: String!) {
    ...
    return entries
      .map(({ node }) => {
        ...
        return {
          ...
          mediaLogo: refs.media_logo || null,
          title: fields.title || '',
          excerpt: fields.excerpt || '',
          publishDate: fields.publish_date || null,
          url: fields.url || null,
          thumbnail: refs.thumbnail || null,
        };
      })
      .sort((a, b) => a.position - b.position);
  } catch (error) {
