 ...
 */
export async function getCollectionByHandle(handle, { revalidate } = {}) {
  const query = `
 ...
 */
export async function getPressPage(language = 'fr', metaobjectType = 'press_page') {
  const lang = SHOPIFY_LANG_MAP[language] || 'FR';
  const query = `
 ...
 */
export async function getPressItems(language = 'fr', metaobjectType = 'press_item') {
  const lang = SHOPIFY_LANG_MAP[language] || 'FR';
  const query = `
...
/**
 * Espaces additionnels de la page Presse (2 sections sous la liste principale).
 * Même structure que press_page / press_item — le client les gère depuis
 * Shopify Admin via les metaobjects :
 *   press_section_2 (en-tête : label, title, intro)
 *   press_section_2_item (éléments : mêmes champs que press_item)
 *   press_section_3 + press_section_3_item (idem)
 * Une section sans en-tête ni élément n'est pas affichée sur le site.
 */
export async function getPressExtraSections(language = 'fr') {
  return Promise.all(
    ['press_section_2', 'press_section_3'].map(async (typeBase) => {
      const [page, items] = await Promise.all([
        getPressPage(language, typeBase),
        getPressItems(language, `${typeBase}_item`),
      ]);
      return { page, items };
    })
  );
...
export async function getCategoryImages() {
  ...
  // Images de catégorie quasi-statiques (gérées depuis Shopify Admin) :
  // ISR 5 min évite de refaire 3 appels frais à chaque rendu et masque
  // les échecs ponctuels de Shopify Storefront.
  const collections = await Promise.all(
    handles.map(handle => getCollectionByHandle(handle, { revalidate: 300 }))
  );

  return handles.reduce((acc, handle, i) => {
    acc[handle] = collections[i]?.image?.url || null;
    return acc;
  }, {});
}
